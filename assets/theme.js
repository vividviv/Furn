/* Furn theme scripts — vanilla ES modules, custom elements, no dependencies. */

const routes = window.Furn?.routes ?? {};
const strings = window.Furn?.strings ?? {};
...
class ProductForm extends HTMLElement {
  ...
  onSubmit = async (event) => {
    if (window.Furn?.cartType !== 'drawer') return;
    event.preventDefault();
...
if (document.body.classList.contains('animate-reveal') && 'IntersectionObserver' in window) {
  ...
  document.addEventListener('tabs:change', scheduleSweep);
...
class CollectionTabs extends HTMLElement {
  ...
  onKeydown = (event) => {
    ...
    let next;
    if (step) next = this.tabs[(this.tabs.indexOf(tab) + step + this.tabs.length) % this.tabs.length];
  ...
  select(tab) {
    ...
    this.dispatchEvent(new Event('tabs:change', { bubbles: true }));
  }
}
